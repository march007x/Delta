...
export function InteractiveUnitCircle({ height = 380 }: { height?: number }) {
  ...
  const draw = useCallback(
    (
      ...
      drawLabel(ctx, vp, cos, sin, `(${fmt(cos)}, ${fmt(sin)})`, {
        color: theme.point,
        bg: theme.bg,
        dx: cos > 0.4 ? -96 : 10,
        dy: sin >= 0 ? -16 : 16,
  ...
  const radLabel = `${fmt(deg / 180, 3)}π`;
      ...
      readout={
        <Readout
          rows={[
            ...
            { label: "cos θ", value: fmt(cos, 4) },
            { label: "sin θ", value: fmt(sin, 4), tone: "delta" },
            { label: "tan θ", value: tanDefined ? fmt(tan, 4) : "ไม่นิยาม" },
