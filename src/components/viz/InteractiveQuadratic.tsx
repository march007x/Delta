...
export function InteractiveQuadratic({ height = 340 }: { height?: number }) {
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, vp: Parameters<typeof drawGrid>[1], theme: Parameters<typeof drawGrid>[2]) => {
      ...
      drawLabel(ctx, vp, h, k, `(${fmt(h, 1)}, ${fmt(k, 1)})`, {
        color: theme.delta,
        bg: theme.bg,
        dy: -16,
      });
  ...
  return (
    <VizFrame
      ...
      readout={
        <Readout
          rows={[
            { label: "จุดยอด", value: `(${fmt(h)}, ${fmt(k)})`, tone: "delta" },
            { label: "แกนสมมาตร", value: `x = ${fmt(h)}` },
            { label: "b² − 4ac", value: fmt(disc) },
            {
              label: "รากของสมการ",
              value: roots ? `${fmt(roots[0] ?? 0)}, ${fmt(roots[1] ?? 0)}` : "ไม่มีรากจริง",
            },
            { label: "ตัดแกน y", value: `(0, ${fmt(c)})` },
            { label: a > 0 ? "ค่าต่ำสุด" : "ค่าสูงสุด", value: fmt(k) },
          ]}
        />
      }
    />
